...
def init_trait(dim, bound, init="random", prng=""):
        if isinstance(init, (int, long, float)):
            ...
            if init.shape != dim: raise \
                ValueError("Shape of array passed to init must be equal to dim.")
            trait = init
        elif init == "random":
...
class population_process:
    ...
    def __init__(self, params):
        ...
        self.death = init_trait((self.N, self.number_of_ages), init=self.init_death,\
                bound=self.bound_death, prng=self.prng)
        ...
        self.n_mut_probs = \
                ...
                np.power((np.zeros(self.number_of_ages*2+1)+self.mrate),\
                np.arange(self.number_of_ages*2+1)) * \
                np.power((np.ones(self.number_of_ages*2+1)-self.mrate),\
                np.arange(self.number_of_ages*2+1)[::-1])
    ...
    def get_agecard(self, ix):
        return my_argmax(self.agebin_mask >= (self.time - self.birth_date[ix]), axis=0,\
                _max=self.number_of_ages-1, subtract=1)
    ...
    def mutate(self, x, mean, bound, dim):
        return np.clip(x + self.prng.normal(mean, self.mrate_std, dim), bound[0], bound[1])
    ...
    def jump_rate_sum(self, where_alive):
        ...
        agecard = my_argmax(np.tile(self.agebin_mask,self.N).reshape(self.N,\
                self.number_of_ages) >= np.repeat(age,self.number_of_ages).reshape(self.N,\
                self.number_of_ages), axis=1, _max=self.number_of_ages-1, subtract=1)
        # sum the jump rates
        ...
        return death_rates + reproduction_rates + self.N*self.eta,\
                np.sum((death_rates.sum(), reproduction_rates.sum(), self.N*self.N*self.eta))
    ...
    def transition(self):
        ...
        how_many_mut = my_argmax(self.AR_birth(ix_in_all, rates[ix]) > u, axis=0,\
                _max=self.number_of_ages*2+1, subtract=0)
        ...
        if how_many_mut < self.number_of_ages*2+1:
            ...
            # NOTE abs here makes sure that when mutation mean is negative
            # (more detrimental mutations), mutation mean for death rate is
            # positive as this corresponds to detrimental mutations
            offspring_death[which_death] = self.mutate(self.death[where_alive][ix][which_death],\
                    abs(self.mrate_mean), self.bound_death, which_death.size)
            offspring_repr[which_repr] = self.mutate(self.reproduction[where_alive][ix][which_repr],\
                    self.mrate_mean,self.bound_repr, which_repr.size)
    ...
    def trajectory(self):
        ...
        for j in range(self.number_of_jumps):
            if self.N > 0:
                self.transition()
                print "Jump: {0}. Time: {1}. Population size: {2}.".format(\
                        j+1, round(self.time, 2), self.N)
            else:
                print "Dieout."
                break
